...
app.post('/api/registrar', async (req, res) => {
  try {
    ...
    await nuevo.save();
    res.json({ ok: true });
  } catch (e) { res.json({ error: "Error interno al registrar." }); }
});
...
// --- FRONTEND CON LÓGICA DE COSTOS DINÁMICOS ---
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="es">
    <head>
      ...
      <style>
        ...
        .btn-primary { background: var(--dark); color: white; border: none; width: 100%; padding: 15px; border-radius: 12px; font-weight: bold; cursor: pointer; font-size: 16px; }
        ...
        .sucursal-card { background: white; padding: 15px; border-radius: 16px; margin-bottom: 10px; border: 1px solid #e2e8f0; }
    ...
    <body>
      ...
      <div id="auth-ui" class="auth-screen">
        <div class="auth-card">
          <h2>BookBarber V2</h2>
          <p style="color: var(--text-muted); margin-bottom: 25px; font-size: 14px;">MedinSur Dev - Gestión Profesional</p>
          <div id="auth-fields">
          ...
          <p style="font-size: 14px; margin-top: 25px;">
            ¿No tienes cuenta? <a href="javascript:toggleV()" id="l-t" style="color:var(--primary); font-weight:700; text-decoration:none;">Registrarme</a>
          </p>
        </div>
      </div>
      ...
      <div id="dash-ui" style="display:none;">
        <div class="navbar">
          ...
          <span id="header-name" style="font-weight:700;">Panel</span>
          <div style="width:22px;"></div>
        </div>

        <div id="menu" class="sidebar">
          <div style="padding:25px; background:var(--dark); color:white; font-weight:800; display:flex; justify-content:space-between;">
          ...
          <div style="margin-top:auto; padding:20px; border-top:1px solid #eee;">
            <div style="font-size:11px; color:var(--text-muted); text-align:center; margin-bottom:10px;">MedinSur Dev © 2026</div>
        ...
        <div class="dashboard-container">
          <div class="plan-card">
            ...
            <button id="btn-pago-dinamico" class="btn-primary" style="background:#ef4444;">Activar Suscripción ($15.000)</button>
          </div>
      ...
      <script>
        ...
        function toggleV() {
          ...
          if(mode === 'reg') {
            fields.innerHTML = \`<input id="r-n" placeholder="Nombre de Barbería"><input id="r-e" placeholder="Email"><input id="r-w" placeholder="WhatsApp (549...)"><input id="r-p" type="password" placeholder="Contraseña">\`;
        ...
        async function procesarAuth() {
          const body = (mode === 'login') ? 
            { email: log-email.value, password: log-pass.value } :
            { nombreNegocio: r-n.value, email: r-e.value, whatsapp: r-w.value, password: r-p.value };
          ...
          } else if (data.ok) { alert("Registro exitoso. Inicia sesión."); location.reload(); }
        ...
        async function load(v) {
          ...
          if(v === 'sucursales') {
            ...
            // Lógica de costo dinámico
            const costoFinal = 15000 + (Math.max(0, list.length - 1) * 5000);
            document.getElementById('btn-pago-dinamico').innerText = \`Activar Suscripción ($\${costoFinal.toLocaleString()})\`;
            
            let h = '<h2>📍 Mis Sucursales</h2>';
            list.forEach(s => h += \`<div class="sucursal-card"><b>\${s.nombre}</b><br><small>\${s.direccion}</small></div>\`);
            
            h += '<hr style="margin:20px 0; border:0; border-top:1px solid #eee;">';
            
            if(list.length === 0) {
              h += '<h3>+ Configurar Sucursal Principal</h3>';
              h += '<p style="color:var(--primary); font-size:14px; font-weight:600;">Esta sucursal ya viene incluida con tu plan base de $15.000.</p>';
            } else {
              h += '<h3>+ Agregar Sucursal Extra</h3>';
              h += '<p style="color:#ef4444; font-size:14px; font-weight:600;">Esta sucursal adicional sumará $5.000 extras a tu cuota mensual.</p>';
            }
            
            h += \`<input id="sn" placeholder="Nombre de la sucursal">
                   <input id="sd" placeholder="Dirección">
                   <button class="btn-primary" onclick="saveS()">Guardar Sucursal</button>\`;
            content.innerHTML = h;
          } else { content.innerHTML = '<h2>🚧 En desarrollo por MedinSur Dev</h2>'; }
        }
...
app.listen(PORT, () => console.log('🚀 BookBarber V2 Full Online'));
