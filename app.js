...
async function obtenerHorariosDisponiblesPublicos({
  ...
  const intervalo = Math.max(
    5,
    Number(datos[0].intervalo_minutos || 30)
  );

  const duracion = Math.max(
    1,
    Number(datos[0].duracion_minutos || 30)
  );
  ...
  const ocupados = turnosExistentes
    .map(turno => {
      ...
      const duracionTurno = Math.max(
        1,
        Number(turno.duracion_minutos || 30)
      );
  ...
  /*
   * BookBarber trabaja con BLOQUES LIBRES REALES.
   *
   * Ejemplo:
   * Jornada: 10:00 a 13:00
   * Turno existente: 11:00 a 11:45
   *
   * Bloques libres:
   * 10:00 a 11:00
   * 11:45 a 13:00
   *
   * El segundo bloque comienza exactamente a las 11:45.
   * No esperamos artificialmente hasta las 12:00.
   */
  for (const rango of rangosLaborales) {
    const inicioLaboral =
      minutosDesdeHora(rango.hora_inicio);

    const finLaboral =
      minutosDesdeHora(rango.hora_fin);

    if (
      inicioLaboral === null ||
      finLaboral === null ||
      finLaboral <= inicioLaboral
    ) {
      continue;
    }

    const ocupadosDelRango = ocupados
      .filter(turno =>
        turno.inicio < finLaboral &&
        turno.fin > inicioLaboral
      )
      .map(turno => ({
        inicio: Math.max(turno.inicio, inicioLaboral),
        fin: Math.min(turno.fin, finLaboral)
      }))
      .sort((a, b) => a.inicio - b.inicio);

    // Unimos ocupaciones que pudieran tocarse o superponerse.
    const ocupadosUnidos = [];

    for (const ocupado of ocupadosDelRango) {
      const ultimo =
        ocupadosUnidos[ocupadosUnidos.length - 1];

      if (
        !ultimo ||
        ocupado.inicio > ultimo.fin
      ) {
        ocupadosUnidos.push({
          inicio: ocupado.inicio,
          fin: ocupado.fin
        });
      } else {
        ultimo.fin = Math.max(
    ...
    // Construimos los espacios realmente libres.
    const bloquesLibres = [];
    let cursor = inicioLaboral;

    for (const ocupado of ocupadosUnidos) {
    ...
    /*
     * Cada bloque libre tiene su propia grilla.
     * Por eso, si un turno termina 11:45,
     * los siguientes horarios pueden ser:
     * 11:45, 12:15, 12:45...
     *
     * Si el bloque comenzó 10:00:
     * 10:00, 10:30...
     */
    for (const bloque of bloquesLibres) {
      for (
        let inicio = bloque.inicio;
        inicio + duracion <= bloque.fin;
        inicio += intervalo
      ) {
        if (
          ahoraMinutos !== null &&
  ...
  return {
    ...
    mensaje:
      horariosUnicos.length > 0
        ? null
        : 'No quedan horarios disponibles para ese servicio en esa fecha.',
